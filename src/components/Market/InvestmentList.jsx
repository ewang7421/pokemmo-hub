...
export const InvestmentList = ({ i, title, fallbackIfEmpty, showTotals }) => {
  ...
  function calculateTotal() {
    const { gain, spent } = investmentsGain.reduce(
      (prev, curr) => ({
  ...
  if (investments.size && i) {
    console.log("single item:");
    investments = new Map([[i, investments.get(i)]])
    console.log(investments);
    if (investments.length === 0 && !fallbackIfEmpty) return false;
  }

  console.log("from");
  console.log(Array.from(investments.entries()).map((entry) => (entry[1])));
    ...
    <Stack gap={2}>
      ...
      <Card>
        ...
        <Table responsive={true} withBaseStyles={{ breakpoint: "920px" }}>
          ...
          <Tbody>
            { Array.from(investments.entries()).map((entry) => {
              console.log("INSIDE");
              return (
                <InvestmentItem
                  key={investments.i}
                  onPriceUpdate={(id, gain, spent) =>
                    addInvestmentGain(id, gain, spent)
                  }
    ...
    <Card>
      ...
      >
        <Typography className="mb-0">
          No investments found. Start now by going to "All Items" and creating
          an investment.
        </Typography>
      </Stack>
    </Card>
